...
ENV_DIR="${ENV_DIR-/opt/huawei/explorer-env/dataset/Common_wl/miniconda3/envs/visual-agent-qwen3vl-sft}"
...
if [[ -n "$CONDA_ENV_PATH" ]]; then
  ...
  if [[ -z "${CC:-}" && -x "$CONDA_ENV_PATH/bin/gcc" ]]; then
    export CC="$CONDA_ENV_PATH/bin/gcc"
  fi
  if [[ -z "${CXX:-}" && -x "$CONDA_ENV_PATH/bin/g++" ]]; then
...
{
  echo "============================================================"
  echo "Visual-agent Qwen3-VL SFT launcher"
