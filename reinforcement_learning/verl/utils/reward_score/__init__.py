...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    elif data_source in ['perception']:
        from . import deepeyesv2
        res = deepeyesv2.compute_score(solution_str, ground_truth, extra_info)
    
    elif data_source in ['reason']:
        from . import deepeyesv2
        res = deepeyesv2.compute_score_math(solution_str, ground_truth, extra_info)
    
    elif data_source in ['search']:
        from . import deepeyesv2
        res = deepeyesv2.compute_score_search(solution_str, ground_truth, extra_info)

    elif data_source in ['vstar-test']:
        from . import deepeyesv2
        res = deepeyesv2.compute_score_acc(solution_str, ground_truth, extra_info, data_source=data_source)

    elif data_source in ['visual-agent-thyme', 'visual-agent-zwz-relation', 'visual-agent-vision-opd', 'visual-agent-deepeyesv2', 'visual-agent-hrbench4k', 'visual-agent-depth-raw', 'visual-agent-tallyqa']:
